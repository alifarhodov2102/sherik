...
export default function PaywallScreen({ route, navigation }: any) {
  ...
  const handleFinalPayment = () => {
    ...
    // Fake the admin verification / processing time
    setTimeout(async () => {
      try {
        const myId = auth.currentUser?.uid;
        if (!myId) throw new Error("No user ID");

        // Create the Chat Room in the database ONLY after they confirm payment!
        ...
        navigation.replace('ChatRoom', { chatId, hostId });
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.content}>
        ...
        <Text style={styles.subtitle}>
          Connect instantly with this roommate. Your fee helps keep SHERIK safe and verified.
        </Text>
      ...
      >
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            {/* Static UZCARD / HUMO Design for Copying */}
            <View style={styles.cardDesign}>
              <Text style={styles.cardLogo}>UZCARD</Text>
              <View style={styles.cardChip} />
              
              <Text style={styles.cardLabel}>Card Number (Long press to copy)</Text>
              {/* selectable={true} allows the user to copy the text natively! */}
              <Text style={styles.cardNumberText} selectable={true}>
                8600 1234 5678 9012
              </Text>
...
const styles = StyleSheet.create({
  ...
  // New static text styles for the card
